...
public:
    ...
    void outbound(const Packet& packet,LspWriter& writer) {
        ...
        else if(method=="textDocument/rename"){result.SetObject();rapidjson::Value changes(rapidjson::kObjectType);std::map<std::string,List> grouped;for(const auto& item:asList(field(body,"changes"))){const auto& edit=asMap(item);grouped[asString(field(edit,"uri"))].push_back(Map{{"range",field(edit,"range")},{"newText",field(edit,"new_text")}});}for(const auto& [uri,edits]:grouped){rapidjson::Value name(uri.data(),static_cast<rapidjson::SizeType>(uri.size()),a),array;toJson(Value(edits),array,a);changes.AddMember(name,array,a);}result.AddMember("changes",changes,a);}
        else result.SetNull();
