...
final class ProjectScaffolder
{
    public function create(string $directory, string $applicationName): array
    {
        ...
        foreach (['app/Actions', 'app/Domains', 'app/Types', 'app/Web', 'config', 'public', 'runtime', 'tests'] as $relative) {
    ...
    public function action(string $project, string $domain, string $name): string
    {
        ...
        $class = str_replace(' ', '', ucwords(str_replace(['.', ':', '-'], ' ', $name))) . 'Action';
        $path = rtrim($project, '/') . '/app/Actions/' . $class . '.php';
        $content = sprintf(<<<'PHP'
<?php
